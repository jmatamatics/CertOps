...
- **Conversational memory**: Add LangGraph checkpointing so a certification architect can iteratively refine artifacts through conversation (e.g., "Add a third skill to the security domain" or "Make that assessment harder"). The graph structure and nodes would remain unchanged; the additions would be a `MemorySaver` checkpointer and thread-based session tracking.
